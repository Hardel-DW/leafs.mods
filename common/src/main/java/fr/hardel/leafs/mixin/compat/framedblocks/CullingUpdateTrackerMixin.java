package fr.hardel.leafs.mixin.compat.framedblocks;
...
/** XFactHD/FramedBlocks, culling updates. Framed blocks enqueue from their region; the level tick walks the positions of the dimension and clears them. */
@Pseudo
@Mixin(targets = "io.github.xfacthd.framedblocks.common.data.cullupdate.CullingUpdateTracker")
public abstract class CullingUpdateTrackerMixin {
    ...
    private static <K, V> Reference2ObjectOpenHashMap<K, V> leafs$sharedUpdatedPositions(Operation<Reference2ObjectOpenHashMap<K, V>> original) {
    ...
    private static LongSet leafs$sharedChunkPositions(long chunk, Operation<LongSet> original) {
