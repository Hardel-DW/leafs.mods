# Config
Leafs crée un fichier de configuration au démarrage, `config/leafs.json`, Au premier démarrage, Leafs met `sync-chunk-writes` à `false` dans `server.properties`. L'admin peut le remettre à true, Leafs n'y touche plus ensuite.
