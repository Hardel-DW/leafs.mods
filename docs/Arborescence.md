...
- `debug/` porte les commandes `/leafs regions`, `timings` et `metrics`, l'affichage côté client et l'analyse de la RAM étant dans le mod à part `Leafs Debug and Metrics`.
...
- `metrics/` mesure le serveur avec les durées par étape de tick du catalogue `TickStages` publié dans le registre `leafs:tick_stage`, les compteurs à fenêtre d'une minute, les pauses des évènements Fabric et les compteurs de reports et de refus du contrat des chunks.
...
- `ticking/` orchestre les régions avec le pool de workers, l'unité de tick sérielle par dimension, l'emprunt de régions par le thread serveur, le budget sériel, le watchdog et les timings.
