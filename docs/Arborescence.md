...
- `chunk/` - Le système de chunks, la boîte aux lettres par chunk, l'attente d'un chunk absent et les chunks que les workers de chunks possèdent.
- `debug/` - La commande `/leafs`
- `entity/` - La snapshot des entités qu'une région tick.
- `global/` - Le moteur des commandes, le moniteur d'état, l'emprunt pendant les évènements de tick de la Fabric API.
