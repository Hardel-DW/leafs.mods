...
2. Chaque région vit à son propre TPS. Un four peut être plus lent d'une région à l'autre par exemple.
3. Toutes les commandes s'exécutent sur le thread serveur, qui emprunte les régions qu'elles touchent. [Vulgarisation.md](Vulgarisation.md)
...
5. Les téléportations et les portails arrivent avec au plus tard au ticks suivant de la régions cible.
6. `END_SERVER_TICK` Les mods qui font leur travail "une fois par tick" via la Fabric API tournent toujours 20 fois par seconde, mais le monde autour n'a pas forcément avancé d'un tick entre deux appels. Une région à 10 TPS a fait un tick sur deux. Un mod qui suppose que tout le monde a tické exactement une fois depuis son dernier appel peut se tromper.
